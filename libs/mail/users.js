...
fns = {
    ...
    changePass: function(email, pass){
        ...
        .then(function(usr){
            if(!usr){
              return Promise.reject(new Error("libs/mail/users#changePass no such user"));
            }
    ...
    createAcct: function(email){
        ...
        if(!email || !re.test(email)){
            return Promise.reject(new Error("libs/mail/users#createAcct email invalid format"));
        }

        return fns.getUser(email)
        .then(function(usr){
            if(usr){
              return Promise.reject(new Error("libs/mail/users#createAcct user already exist"));
            }
