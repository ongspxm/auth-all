...
function getDB(){
    return (new sqlite.Database(process.env.DB_DBASE));
}
...
module.exports = {
    ...
    select: function (table, where, params, fields) {
        return new Promise((res, err) => {
            ...
            var sql = "SELECT "+vals+" FROM "+table;
    ...
    setup: function(){
        return new Promise((res, err) => {
            fs.readFile(process.env.DB_SCHEMA, function(err, data){
                module.exports.exec(data.toString()).then(() => res());
            });
        });
    },
